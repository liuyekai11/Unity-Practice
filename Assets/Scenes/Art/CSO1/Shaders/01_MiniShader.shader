Shader "Shader Base " //Shader的真正名字  可以是路径式的格式
{
	/*材质球参数及UI面板
		...
		Pass 
		{
			...
			*/
			...
			v2f vert (appdata v)
			{
				...
				//o.pos = UnityObjectToClipPos(v.vertex);//模型空间转换到摄像机空间
				o.uv = v.texcoord0 * _MainTex_ST.xy + _MainTex_ST.zw;//贴图缩放乘法
